...
export default function StopMap({ ride, stops, onAddStop, onRemoveStop, height = 340 }: Props) {
  ...
  useEffect(() => {
    ...
    import('leaflet').then(L => {
      ...
      // Route line if both exist
      if (fromCoords && toCoords) {
        L.polyline([fromCoords, toCoords], { color: '#94A3B8', weight: 2, dashArray: '6 4' }).addTo(map)
        map.fitBounds(L.latLngBounds([fromCoords, toCoords]), { padding: [40, 40], maxZoom: 14 })
      }
    ...
    return () => {
      cancelled = true
      if (mapRef.current) { mapRef.current.remove(); mapRef.current = null; stopMarkers.current = [] }
    }
  }, [ride])
